...
{{- if and (and (or (not (regexMatch "^(0|[1-9]\\d*)\\.(0|[1-9]\\d*)\\.(0|[1-9]\\d*)(?:-((?:0|[1-9]\\d*|\\d*[a-zA-Z-][0-9a-zA-Z-]*)(?:\\.(?:0|[1-9]\\d*|\\d*[a-zA-Z-][0-9a-zA-Z-]*))*))?(?:\\+([0-9a-zA-Z-]+(?:\\.[0-9a-zA-Z-]+)*))?$" $version)) (semverCompare ">=2.2.0-0" $version)) (not .Values.platform.tenant.license) (eq .Values.platform.tenant.mode "ONPREM"))  -}}
{{- fail ".Values.platform.tenant.license missing. A license is required to run steadybit platform >= 2.2.0" -}}
{{- end -}}
{{- $version -}}
{{- end -}}